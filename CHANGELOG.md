...
- 自動生成バックアップ4種の件数、保持上限、最新世代、除外件数、保持状態を読み取り専用CLIで確認できるようにした。
- KOT同期対象部門をadminモードの管理画面から追加・無効化・再有効化・削除できるようにし、設定変更後のWebサービス再起動を不要化。

### Changed

- 社員DBの総数・有効数・無効数とemployeeKey.csvの実レコード数、整合性件数を検証ログへ明示。


### Fixed

- 本番deployでDBを事前バックアップしてから既存スキーマを更新し、更新後のDB検証より前に安全なマイグレーションを完了するよう修正。
- deploy前DB・社員削除前・employee CSV置換前の自動生成バックアップを安全に最新30件へ整理し、手動バックアップや想定外ファイルを自動削除しないよう改善。
- deployのhealth待機中は一時的な`curl`接続エラーを表示せず、全試行失敗時の診断出力は維持することをテストと運用文書で明確化。
