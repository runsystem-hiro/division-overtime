...
- 残業通知で当月の深夜残業が1分以上ある場合に、残業時間の横へ`｜🌙 H:MM`を表示するようにしました。
- KOT社員同期プレビューへ業務委託の件数表示と、初期非表示の表示フィルターを追加しました。

### Changed

- KOT同期対象部門カードを広い画面でも最大3列に調整しました。
- 公開バージョンを2.4.2へ更新しました。

## [2.4.1] - 2026-08-03

- KOT同期対象部門をレスポンシブなカードグリッドへ変更し、有効・無効を確認ダイアログなしで切り替えられる状態トグルへ改善。

### Added

- KOT同期対象部門をadminモードの管理画面から追加・無効化・再有効化・削除できるようにし、設定変更後のWebサービス再起動を不要化しました。
- 自動生成バックアップ4種の件数、保持上限、最新世代、除外件数、保持状態を読み取り専用CLIで確認できるようにしました。

### Changed

- 社員DBの総数・有効数・無効数と`employeeKey.csv`の実レコード数、整合性件数を検証ログへ明示しました。
- Cloudflare Access利用時のviewer自動認証、adminへの一時昇格、viewerへの降格と、ローカルアカウント認証の違いをREADMEとAPIドキュメントへ明記しました。
- 正式リリース確認に自動生成バックアップの保持状態診断を追加しました。
- 公開バージョンを2.4.1へ更新しました。

### Fixed

- 本番deployでDBを事前バックアップしてから既存スキーマを更新し、更新後のDB検証より前に安全なマイグレーションを完了するよう修正しました。
- deploy前DB・社員削除前・employee CSV置換前の自動生成バックアップを安全に最新30件へ整理し、手動バックアップや想定外ファイルを自動削除しないよう改善しました。
- deployのhealth待機中は一時的な`curl`接続エラーを表示せず、全試行失敗時の診断出力は維持することを回帰テストで固定しました。
...
- 管理者パスワードによる30分間のadmin昇格と、viewerへの明示的な降格を追加した

### Changed

- アカウントメニューに認証元のユーザー情報とモード切替を追加し、ログアウトを専用アイコンへ変更した
- 管理者モードモーダルの閉じる操作を×アイコンへ統一し、ボタンの主従とフォーカス復帰を改善した

## [2.3.0] - 2026-08-01

### Added

- 通知実行履歴を最新順の20件固定で取得するAPIページネーションと、前後ページ操作を追加しました。
- 社員一覧と通知実行詳細に、PC列見出しとモバイル操作から使えるソート機能を追加しました。

### Changed

- 通知実行履歴APIに`limit`と`offset`を追加し、全件取得ではなくSQLite側で必要なページだけを取得するよう変更しました。
- 通知実行履歴の再読込、詳細表示、PC・モバイル表示で現在ページを維持するよう整理しました。
- READMEと関連する`docs/`をv2.3.0の仕様へ更新し、公開例のユーザー名・部署コード・社員番号を汎用値へ置き換えました。
- 公開バージョンを2.3.0へ更新しました。

## [2.2.2] - 2026-07-30

### Added

- Web管理UIに一般ユーザー向け閲覧専用ロールを追加し、社員・同期・通知履歴の参照を許可しました。
- Web管理UIに共通トーストと確認ダイアログを追加し、成功・エラー・危険操作の表示を統一しました。
- 成功通知の自動消去、ホバー中の停止、残り時間表示、reduced motion対応を追加しました。
- 通知実行詳細にエラー詳細モーダルを追加し、エラー全文の確認・選択・コピーに対応しました。

### Changed

- 更新系APIと整合性確認・KOT取得を管理者専用にし、フロントエンドでも一般ユーザーの対象操作を無効化しました。
- Web管理UIのトースト外観、無効・処理中ボタン、アンビエント背景、ページ遷移、スケルトン表示を統一し、読み込み状態を明確化しました。
- アンビエント背景の移動量・透明度変化・周期を調整し、可読性を保ちながら動きを認識しやすくしました。
- 通知実行履歴・詳細に社員氏名を表示し、モバイルカードの情報密度を改善しました。
- 通知実行詳細の列順・改行・長文表示を整理し、送信先、重複元、Slack timestampの一覧性を改善しました。
- 公開バージョンを2.2.2へ更新しました。

### Fixed

- iPhone Safariで長いモーダルが画面外へ外れる問題を修正し、モバイルでは上端寄せ・内部スクロール・背景スクロール固定へ変更しました。
- ヘッダーのシステム状態とアカウントメニューをPortal描画へ変更し、重なりとログアウト不能を修正しました。
- システム状態とアカウントメニューを排他表示にし、開いているメニューを常に前面で操作できるようにしました。
- アンビエント背景の明るさを抑え、淡い光源が画面外周を不規則に巡る動きへ変更しました。

## [2.2.1] - 2026-07-29

### Changed

- 本番環境を青緑、開発環境を黄橙で表示し、環境バッジの意味と視覚的な印象を一致
- test環境と未知の環境値を個別ラベル・配色で表示し、開発環境への誤分類を防止
- frontend testのhealthバージョンを`frontend/package.json`から参照し、リリース時の更新漏れを防止
- 公開バージョンを2.2.1へ更新
...
- READMEに社員管理、KOT社員同期、通知履歴の現行画面画像を追加
- Web管理UIに通知実行履歴の一覧・詳細表示を追加
- 通知実行履歴に`timer` / `manual` / `test` / `unknown`の実行元を記録し、APIとWeb UIへ表示

### Changed

- Web管理UIの左サイドバーを廃止し、環境表示、主要画面ナビゲーション、health、アカウント操作を共通ヘッダーへ統合
- 社員管理、KOT同期、通知履歴の見た目と操作導線を共通デザイントークンで統一
- 社員管理に結果件数、検索条件、条件別の空状態を追加
- KOT同期に集計、判定別フィルター、一括選択、選択内訳、反映件数表示を追加
- 通知履歴に実行結果集計、日本語ラベル、一部失敗表示、送信先別詳細を追加
- PCとタブレットではテーブル表示を維持し、モバイル幅でカード表示へ切り替えるレスポンシブ構成へ調整
- ヘッダー操作部とテーブルの文字サイズ、情報密度、余白を調整
- READMEと`docs/`をv2.2.0の実装・運用へ更新
- 公開バージョンを2.2.0へ更新

### Fixed

- モバイルのKOT同期カードでhover時にセル背景が縞模様になる表示を修正
- `scripts/deploy.sh`で全systemd unitを本番へ反映し、リポジトリ内定義との一致を検証
- `scripts/deploy.sh`のWeb起動待機中は一時的な`curl`接続エラーを抑制し、最終失敗時のみ表示
...
- Web認証、SQLite社員管理、employeeKey.csv再生成、社員管理Web UI、KOT社員同期をv2.0.0の正式範囲として確定
- VERSION、Python、フロントエンドのバージョンを2.0.0へ統一し、整合性検証スクリプトを追加
- deploy.shでAPIヘルスチェックの稼働バージョン一致を検証
- Windows検証、Raspberry Pi実機確認、Web停止時の通知独立性、ロールバック、タグ・Release作成のチェックリストを追加
- 社員削除バックアップを含む復旧手順を運用文書へ追加

- 社員編集画面から、削除前バックアップ・CSV再生成・整合性維持を伴う社員削除に対応。
- KOT退職済みかつSQLite未登録の社員を同期候補から除外。
- KOT同期で無効社員の明示的な再有効化と、適用前バックアップの30世代管理に対応。

### Added

- KOT社員同期履歴へバックアップ先を保存し、管理画面の最終実行欄で確認できるようにした
...
- KOTに存在しない社員またはKOT退職済み社員がすでに無効な場合、KOT社員同期の無効化候補へ繰り返し表示しないようにした
...
- KOT同期バックアップのディレクトリを`0700`、SQLiteとCSVを`0600`に制限
...
- ローカル開発時だけ有効にできる通知履歴の読み取り専用サンプル表示を追加し、実データがない環境でも一覧・詳細・失敗・重複スキップなどを画面確認できるようにしました。
