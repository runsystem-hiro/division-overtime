...
- 通知実行詳細の列順・改行・長文表示を整理し、エラー全文を専用詳細からコピーできるように改善

- 通知実行履歴・詳細に社員氏名を表示し、モバイルカードの情報密度を改善しました。

### Fixed

- iPhone Safariで長いモーダルが画面外へ外れる問題を修正し、モバイルでは上端寄せ・内部スクロール・背景スクロール固定へ変更
- ヘッダーのシステム状態とアカウントメニューをPortal描画へ変更し、重なりとログアウト不能を修正
- アンビエント背景の明るさを抑え、淡い光源が画面外周を不規則に巡る動きへ変更

### Changed

- Web管理UIのトースト外観、無効・処理中ボタン、アンビエント背景、ページ遷移、スケルトン表示を統一し、読み込み状態を明確化。
- アンビエント背景の移動量・透明度変化・周期を調整し、可読性を保ちながら動きを認識しやすくしました。

### Fixed

- システム状態ポップオーバーとアカウントメニューの重なり順を整理し、ログアウト操作が他要素に遮られる問題を修正しました。
...
- 更新系APIと整合性確認・KOT取得を管理者専用にし、フロントエンドでも対象操作を無効化しました。
