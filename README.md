...
- KOT KeyはWeb APIのレスポンスや画面へ表示しない
- KOT社員同期、CSV生成、通知処理のSQLite切替は後続PRで実装
...
Web管理UIは既存のthreshold、weekly、health処理から独立したFastAPIサービスとして起動します。認証基盤と社員SQLite管理基盤まで実装済みです。社員管理画面、KOT社員同期、CSV生成は後続PRで追加します。
