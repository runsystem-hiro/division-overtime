...
- 重複スキップ時はdedupe keyと重複元runを詳細画面から確認できます。
...
v2.1.0のWindows検証、Raspberry Pi実機確認、Web停止時の通知独立性確認、ロールバック、タグ・GitHub Release作成は[リリースチェックリスト](docs/release-checklist.md)を使用します。
...
frontendの型チェックにはTypeScript 6.0系を使用します。TypeScript 7への移行準備として、非推奨設定を`ignoreDeprecations`で隠さず、`tsconfig`を必要最小限の構成に保ちます。
