...
Pull Request作成時と`main`へのpush時に、`.github/workflows/ci.yml`で次を自動実行します。
...
CIは検証専用です。Raspberry Piへのデプロイ、KOT APIへの接続、Slack通知、タグ作成、GitHub Release作成は行いません。ローカルでの事前検証も引き続き実行してください。
...
- 必須チェック`CI / verify`が成功してからsquash mergeする
- `main`へのforce pushとブランチ削除は禁止する
- merge commitとrebase mergeは使用せず、squash mergeのみ使用する

ローカル検証は問題を早期発見するため、CIはクリーンなGitHub Actions環境で同じ主要検証を再確認するために使用します。CI成功はローカル検証の代替ではありません。
...
v2.0.2のWindows検証、Raspberry Pi実機確認、Web停止時の通知独立性確認、ロールバック、タグ・GitHub Release作成は[リリースチェックリスト](docs/release-checklist.md)を使用します。
...
開発用通知先は全部署共通の`ALL`を`developer@example.com`だけに固定し、
