...
Web管理UIの起動には`.env`の管理者認証・セッション設定が必要です。任意で閲覧専用ユーザーを設定でき、社員・同期・履歴を閲覧できますが、社員変更、整合性の再確認、KOT取得・同期反映はUIとAPIの両方で禁止されます。画面URLは`/`（社員管理）、`/kot-sync`（KOT同期）、`/notifications`（通知履歴）です。FastAPIのSPAフォールバックにより各URLを直接開けます。共通ヘッダーには環境表示、主要画面ナビゲーション、health表示、アカウント操作を集約しています。
...
- ログイン画面と管理画面では、可読性を保った控えめなアンビエント背景をCSSで表示します。
- 社員・同期・履歴の切り替えは短いクロスフェードを使用し、`prefers-reduced-motion` では移動を抑制します。
- 初回読み込みは対象領域のスケルトン、再読み込みは既存データを維持した更新中表示として区別します。
- 権限不足などの無効ボタンは `not-allowed`、処理中ボタンは専用の進行表示で区別します。
