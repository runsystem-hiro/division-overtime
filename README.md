...
社員管理画面からKOT社員差分を取得・確認し、選択した差分をSQLiteと`employeeKey.csv`へ反映できます。同期処理は二重起動を拒否し、API利用禁止時間帯（08:30〜10:00、17:30〜18:30）は開始できません。同期前にはSQLiteとCSVのバックアップを作成し、CSVは原子的に再生成します。画面には最終実行日時と新規・更新・無効化件数を表示します。KOT KeyやAPIトークンは表示しません。
...
社員追加・編集時は次の順序で処理します。保存APIは、保存された社員情報に加えて、`employeeKey.csv`の生成成否・生成日時・出力件数・出力先を返します。CLIの`employees export-csv --apply`も同じ生成結果を標準出力へ記録します。成功・失敗はアプリケーションログにも記録しますが、KOT Keyは応答・標準出力・ログに含めません。
