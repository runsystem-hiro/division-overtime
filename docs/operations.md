...
Slack上の実表示確認のために`--dry-run`なしでweeklyを手動実行すると、その週の重複防止キーを消費する。同じ週の正規weeklyが対象受信者だけスキップされるため、実送信テストを行う場合は事前にSQLiteをバックアップし、終了後に`docs/release-checklist.md`の「本番Slack表示確認を伴う通知テスト」に従って対象履歴を確認する。
